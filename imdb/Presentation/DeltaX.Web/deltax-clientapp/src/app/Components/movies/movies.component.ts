...
export class MoviesComponent extends Utilities implements OnInit {
  ...
  editModal(id: number) {
    ...
    if (!super.isNullOrEmpty(this.movieInEdit)) {
      ...
      this.selectedProducers = this.allProducers.map(j => {
        if (j.producerID == this.movieInEdit.producerID) {
          return { "id": j.producerID, "itemName": j.producerName }
        }
      });
      this.selectedActors = this.allActors.map(j => {
        if (actorsInMovie.indexOf(j.actorID) !== -1) {
          return { "id": j.actorID, "itemName": j.actorName }
        }
      });
  ...
  saveData(val: any) {
    ...
    this.movieInEdit.actors = this.selectedActors.map(j => {
      return {
        actorID: j.id,
        actorName: j.itemName
      }
    });
    var hasProducer = this.isNullOrEmpty(this.selectedActors);
